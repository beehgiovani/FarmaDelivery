...
- Painel admin exibe falha ao abrir comprovante como feedback inline no historico, sem `alert` modal bloqueando a operacao.
- CSS antigo de mapa simulado foi removido do admin, mantendo apenas Leaflet real e estado vazio honesto quando nao houver pontos.
...
- Resumo `docs/production-readiness.md` criado para separar prontidao local automatizada das validacoes que ainda dependem de ambiente real.
- README raiz alinhado aos comandos `npm run mobile:local-check`, `--api-config`, resumo de prontidao e checklists de smoke test.
