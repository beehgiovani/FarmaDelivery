...
7. Se preferir validar por partes, rodar `npm run secret:session:self-test`, `npm run env:production:check -- file=env/api.env file=env/admin.env`, `npm run repo:sensitive:check:self-test`, `npm run repo:sensitive:check`, `npm run db:rls:check`, `npm test` e `npm run build`.
