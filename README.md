...
Listar a ordem do preflight sem executar:
...
6. Rodar `npm run preflight:production -- file=env/api.env file=env/admin.env`, que executa autoteste do gerador de segredo, checagem de ambiente, autoteste/checagem de arquivos sensiveis versionados, RLS, testes e build sem imprimir valores sensiveis.
7. Se preferir validar por partes, rodar `npm run secret:session:self-test`, `npm run env:production:check -- file=env/api.env file=env/admin.env`, `npm run repo:sensitive:check:self-test`, `npm run repo:sensitive:check`, `npm run db:rls:check:self-test`, `npm run db:rls:check`, `npm test` e `npm run build`.
